  ...
  )
  {
    ...
    foreach (var resource in resources)
    {
      ...
      double nexusToBuild = Math.Sqrt(
        Math.Pow(buildPixelPos.X - nexusPos.X, 2) + Math.Pow(buildPixelPos.Y - nexusPos.Y, 2)
      );
      double buildToResource = Math.Sqrt(
        Math.Pow(resourcePos.X - buildPixelPos.X, 2) + Math.Pow(resourcePos.Y - buildPixelPos.Y, 2)
      );
